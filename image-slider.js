...
H5P.ImageSlider = (function ($) {
  ...
  function C(options, id) {
    ...
    this.options.imageSlides = this.options.imageSlides.filter((slide) => slide.params && slide.params.image && slide.params.image.params && slide.params.image.params.file);
    ...
    this.on('resize', () => {
      const fullScreenOn = self.$container.hasClass('h5p-fullscreen') || self.$container.hasClass('h5p-semi-fullscreen');
      if (fullScreenOn) {
        self.$slides.css('height', '');
        const newAspectRatio = window.innerWidth / (window.innerHeight - self.$progressBar.outerHeight());
        for (let i = 0; i < self.imageSlides.length; i++) {
          self.imageSlides[i].setAspectRatio(newAspectRatio);
        }
      }
      else if (self.aspectRatio && self.$slides) {
  ...
  C.prototype.determineAspectRatio = function () {
    ...
    switch (this.options.aspectRatioMode) {
      case 'auto':
        var imageRatios = [];
        for (let i = 0; i < this.options.imageSlides.length; i++) {
          const imageFile = this.options.imageSlides[i].params.image.params.file;
          imageRatios[i] = imageFile.width / imageFile.height;
        }
        imageRatios.sort((a, b) => a - b);
  ...
  C.prototype.loadImageSlides = function () {
    ...
    for (let i = this.currentSlideId; i < this.imageSlides.length && i < this.currentSlideId + 3; i++) {
  ...
  C.prototype.attachControls = function () {
    ...
    C.handleButtonClick(this.$leftButton, () => {
      if (!self.dragging) {
        self.gotoSlide(self.currentSlideId - 1);
      }
    });

    C.handleButtonClick(this.$rightButton, () => {
      if (!self.dragging) {
        self.gotoSlide(self.currentSlideId + 1);
      }
    });
  ...
  C.prototype.gotoSlide = function (slideId) {
    ...
    const nextSlideDirection = (this.currentSlideId < slideId) ? 'future' : 'past';
    const prevSlideDirection = nextSlideDirection === 'past' ? 'future' : 'past';
    this.currentSlideId = slideId;
    this.loadImageSlides();
    const $prevSlide = this.$currentSlide;
    const $nextSlide = (this.imageSlideHolders[slideId]);
    if (!this.dragging) {
  ...
  C.prototype.updateProgressBar = function () {
    ...
    const heightInPercent = this.$currentSlide.height() / this.$slides.height() * 100;
  ...
  C.prototype.initDragging = function () {
    const self = this;
    this.$slidesHolder.on('touchstart', (event) => {
      self.dragging = true;
    ...
    this.$slidesHolder.on('touchmove', (event) => {
      event.preventDefault();
      self.dragActionUpdate(event.originalEvent.touches[0].pageX);
    });

    this.$slidesHolder.on('touchend', (event) => {
    ...
    this.$slidesHolder.on('touchcancel', () => {
      self.finishDragAction();
      self.dragStartTime = false;
    });
  };
  ...
  C.prototype.dragActionUpdate = function (x) {
    ...
    if (this.currentSlideId > 0) {
      const $prevSlide = this.imageSlideHolders[this.currentSlideId - 1].addClass('h5p-image-slider-dragging');
    ...
    if (this.currentSlideId < this.imageSlideHolders.length - 1) {
      const $nextSlide = this.imageSlideHolders[this.currentSlideId + 1].addClass('h5p-image-slider-dragging');
      if (this.dragXMovement > 0) {
  ...
  C.prototype.finishDragAction = function () {
    ...
    const xInPercent = this.dragXMovement / this.$currentSlide.width();
    if (xInPercent < -0.3) {
      if (this.currentSlideId < this.imageSlideHolders.length - 1) {
